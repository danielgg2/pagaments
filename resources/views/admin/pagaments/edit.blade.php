...
@section('content')
	...
	<form action="{{ url('/administracio/pagaments/editar/'.$pagament->id) }}" method="post">
	{{ csrf_field() }}
	{{ method_field('PUT') }}
    ...
    <input type="hidden" name="usuari_id" value="{{ auth()->id() }}">
		...
		<select name="nivell" value="{{ $pagament->nivell }}">
    <?php $nivells = array("ESO", "BAT", "CF", "PR"); ?>
    @foreach($nivells as $key => $nivell)
      @if($nivell === $pagament->nivell)
    ...
    @endforeach
		...
		<input type="text" name="comanda" value="{{$pagament->comanda}}"><br>
    ...
    @endif
		...
		<input type="text" name="titol" value="{{$pagament->titol}}"><br>
    @if ($errors->has('titol'))
            <br><span class="alert alert-danger">
    ...
    @endif
		...
		<textarea class="my-editor" name="descripcio">{{$pagament->descripcio}}</textarea><br>
    ...
    @endif
		...
		<input type="number" step="0000.01" name="preu" value="{{$pagament->preu}}"><br>
    @if ($errors->has('preu'))
            <br><span class="alert alert-danger">
    ...
    @endif
		...
		<input type="date" name="data_inici" value ="{{$pagament->data_inici}}"><br>
    ...
    @endif
		...
		<input type="date" name="data_fi" value ="{{$pagament->data_fi}}"><br>
    @if ($errors->has('data_fi'))
            <br><span class="alert alert-danger">
                <strong>{{ $errors->first('data_fi') }}</strong>
            </span><br><br>
    @endif
		<label for="estat">Estat</label><br>
		<select name="estat">
            	<option value="Actiu">Actiu</option>
            	<option value="Inaciu">Inaciu</option>
		</select><br><br>
